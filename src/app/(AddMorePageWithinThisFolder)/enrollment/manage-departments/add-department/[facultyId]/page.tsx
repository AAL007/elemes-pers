...
const AddDepartment = ({params} : {params : {facultyId: string}}) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    let department: MsDepartment = {
      DepartmentId: generateGUID(),
  ...
  return (
    <PageContainer title="Add Department" description="Add Department">
      <>
      <Modal
        backdrop="blur"
        isDismissable={false}
            ...
            <>
              ...
              <ModalFooter>
                ...
                <Button color="primary" onPress={async() => {
                    ...
                    try{
                        await deleteDepartmentCourse(departmentId, courseId).then((object: any) => {
                            if(object.statusCode == 200){
                                onClose();
                                setCourseId("");
                                fetchDepartmentCourses(departmentId).then((object: any) => {
                                    setDepartmentCourse(object.data || []);
                                })
                                setErrorMessage("");
                                }else{
      ...
      <Box component="div">
        <div>
          <form onSubmit={handleSubmit}>
            <h2 style={{ fontSize: "22px", marginBottom: "20px", fontWeight: "600"}}>Add Department</h2>
            <Grid container className="mt-0.5">
                ...
                <Grid item xs={3} sm={3} md={3} lg={3} className="mt-4 flex justify-end">
                    <Button color="primary" variant="solid" className="flex w-full sm:max-w-[60%] h-14 mb-10" isLoading={isLoading} onClick={(e: any) => {
                        setLoadingStatus(true)
                        handleSubmit(e)
                    }}>
                        Add Department
