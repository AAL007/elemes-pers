...
export async function uploadFileToAzureBlobStorage(containerName: string, file: File, courseName: string, sessionId: string) {
...
export async function replaceFileInAzureBlobStorage(containerName: string, file: File, courseName: string, sessionId: string) {
    const blobServiceClient = new BlobServiceClient(`${azureStorageUrl}?${sasToken}`)
    let containerClient = blobServiceClient.getContainerClient(containerName);
...
export async function deleteFileInAzureBlobStorageByUrl(containerName: string, courseName: string, sessionId: string): Promise<void> {
    const blobServiceClient = new BlobServiceClient(`${azureStorageUrl}?${sasToken}`)
    let containerClient = blobServiceClient.getContainerClient(containerName);
    const prefix = `${courseName}/${sessionId}`;
    let oldBlobName: string | undefined;
